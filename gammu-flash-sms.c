...
int main(int argc, char **argv)
{
	...
	GSM_SMSC PhoneSMSC;
	//char recipient_number[sizeof(argv[1]) + 1];
	//strcpy(recipient_number, argv[1]);
	char *recipient_number = argv[1];
	//char message_text[sizeof(argv[2]) + 1];
	//strcpy(message_text, argv[2]);
	char *message_text = argv[2];
	unsigned char message_unicode[(sizeof(message_text) + 1) * 2];
	GSM_Debug_Info *debug_info;
	int return_value = 0;

	printf("Sending \"%s\" to %s", message_text, recipient_number);
